...
interface Options {
  ...
  selectedIds: string[];
  searchExclude?: Record<string, string | number | boolean>;
}

const getFilterParser =
  (user: CurrentAdmin, record: RecordJSON) =>
  (filters: Options['filters']): Record<string, string> => {
    ...
    Object.entries(filters).forEach(([key, filter]) => {
      switch (filter.type) {
        case 'owner': {
          const { source } = filter;
          const value =
            (source ? (record.params[source] as string | undefined) : undefined) ?? user.id;
...
export const useFetchOptions = ({
  ...
  const user = useSelector<ReduxState, CurrentAdmin>((s) => s.session);
    ...
    try {
      const response = await new ApiClient().resourceAction({
        ...
        params: {
          ...
          ...(input ? { [`filters.${searchProperty}`]: input } : {}),
          ...searchExclude,
        },
      });
      const records = response?.data?.records;
      const incomingRecords =
        (currentResource === resourceId ? records?.filter?.((r) => r.id !== record.id) : records) ??
        [];
      setCache((prev) => {
        const copy = { ...prev };
  ...
  return {
    search,
    error,
    selected: selectedIds.map((selectedId) => cache[selectedId]),
  };
};
