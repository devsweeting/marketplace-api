...
import { RefreshRequestDto } from 'modules/users/dto/refresh-request.dto';
...
describe('UserController (e2e)', () => {
  ...
  describe('OTP flow', () => {
    ...
    //   //expect(userOtp.used).toBeTruthy(); //POTENTIAL ERROR - The OTP token should be marked as used after it confirm login.
    ...
    test('should test that a refresh token returns a new access token for login', async () => {
      ...
      //check that are refresh token is assigned
      expect(loggedInUser.refreshToken).toBeDefined();
      ...
      await request(app.getHttpServer())
        ...
        .expect(({ body }) => {
          ...
          // expect(body.user).toEqual(loggedInUser); //ERROR Dates return a string "", the other doesn't.
